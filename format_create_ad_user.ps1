      ...
      Else
      {
        ...
        If(!$exists)
        {
          ...
          Try
          {
            ...
            # Add-ADGroupMember -Identity "Mednet_Users, MedNet_VPN" -Members $sam

            ##
            ## My loop.  Change group names here to add the user. 
            $groupsToAdd = ("Mednet_Users", "MedNet_VPN")
            for ($i = 0; $i -lt 2; $i++)
            {
              Add-ADGroupMember -Identity $groupsToAdd[$i] -members $sam
            }
            ##
            ##
